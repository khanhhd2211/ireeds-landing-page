...
async function getResearchPapers(): Promise<ResearchPaper[]> {
  try {
    ...
    const result = await client.query(
      "SELECT * FROM research_paper WHERE fields LIKE '%sustainable-economy%' ORDER BY id"
    )
    client.release()
    return result.rows
  } catch (error) {
...
export default async function SustainableEconomyPage() {
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="mb-16 text-center">
            <h1 className="text-5xl md:text-6xl font-bold text-foreground mb-4 tracking-tight">
              Sustainable Economy Research
            </h1>
            <p className="text-foreground/70 text-lg max-w-3xl mx-auto">
              Explore our research publications on sustainable economic development, environmental economics, and climate change impacts.
            </p>
          </div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {researchPapers.map((paper: ResearchPaper, index: number) => (
              <div key={index} className="flex flex-col bg-card border border-border rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
                {/* Paper Image */}
